...
import { formatPipelineResult, parsePipeline } from '../module/node/api/nodeparese';
...
const SubmitButton = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        const message = formatPipelineResult(result);
        alert(message);
      } else {
  ...
  return (
    <button
      ...
      className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-50 px-12 py-4 text-base font-bold text-white rounded-[50px] border border-white/30 outline-none transition-all duration-300 flex items-center gap-2.5 ${
        loading
          ? 'bg-gradient-to-br from-purple-500/40 to-pink-500/40 opacity-60 cursor-not-allowed'
          : 'bg-gradient-to-br from-purple-500/80 to-pink-500/80 hover:-translate-y-0.5 hover:scale-[1.02] cursor-pointer'
      }`}
      style={{
        boxShadow: loading
